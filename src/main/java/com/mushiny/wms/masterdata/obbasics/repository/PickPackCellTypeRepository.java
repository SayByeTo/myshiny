...
//            " )" +
//            " order by u.name ")
    @Query(" select w from PickPackCellType w " +
        " where w.entityLock = :entityLock " +
        " and not exists (" +
        " select 1 from BoxType c " +
        " left join c.pickPackCellTypes cw " +
        " where cw.id = w.id " +
        " and c.id = :boxTypeId) " +
        " order by w.name")
    List<PickPackCellType> getUnassignedReBinCellTypes(@Param("boxTypeId") String boxTypeId,
